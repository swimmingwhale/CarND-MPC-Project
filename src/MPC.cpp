...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
	...
	//为了防止急转弯,让车辆平稳运行,随意对转角与加速度大小进行限制
    for (unsigned int i = 0; i< N - 1; i++) {
      fg[0] += 1000*CppAD::pow(vars[delta_start + i], 2);
      fg[0] += CppAD::pow(vars[a_start + i], 2);
    }

	//让转角与加速度平滑过渡,以便车辆运行平稳
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // model variables (includes both states and inputs).
  ...
  /** ipopt 需要的参数:
  	options  ...
  	vars  变量
  	vars_lowerbound 变量下限
	vars_upperbound  变量上限
	constraints_lowerbound  限制下界
	constraints_upperbound  限制上界
	fg_eval  拟合多项式
  	solution  解
  **/
  ...
  // One node in advance, in order to handle the Latency
